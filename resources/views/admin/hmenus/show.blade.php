            ...
            <div class="card rounded-0 shadow-sm">
                <div class="card-header">
                    ...
                    <div class="card-header-actions">
                        <a href="#create-menu-item" data-toggle="modal"
                            class="card-header-action btn btn-sm btn-primary text-light">Add New</a>
                    </div>
                </div>
                <div class="card-body">
                                    ...
                                    <tr data-index="{{ $item->id }}" data-order="{{ $item->order }}">
                                        ...
                                        <td>
                                            <form action="{{ route('admin.menuItems.destroy', $item->id) }}" method="post">
                                                ...
                                                @method('DELETE')
                                                <div
                                                    class="btn-group btn-group-sm btn-group-inline d-flex justify-content-between">
                                                    <a href="#edit-menu-item" data-toggle="modal"
                                                        data-route="{{ route('admin.menuItems.update', $item->id) }}"
                                                        data-title="{{ $item->title }}" data-url="{{ $item->url }}"
                                                        class="btn btn-success rounded-0 mr-1">Edit</a>
                                                    <button type="submit"
                                                        class="btn btn-danger rounded-0 ml-1">Delete</button>
                                                </div>
                                            </form>
                                        </td>
                                    </tr>
