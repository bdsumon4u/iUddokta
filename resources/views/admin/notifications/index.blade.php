                ...
                @forelse($notifications as $notification)
                    ...
                    <div class="shadow-sm card rounded-0">
                        <div class="card-header p-2 {{ $unread ? 'border-red' : 'border--green' }}">
                            <span class="p-1 bg-secondary text-light" style="width: 60px; display: inline-block; text-align: center;">Arrived</span> {{ $notification->created_at->format('F j, Y - h:i A') }}
                            <span class="float-right">{{ $notification->created_at->format('h:i A') }}</span>
                        </div>
                        @unless($unread)
                        <div class="p-2 card-header">
                            <span class="p-1 bg-secondary text-light" style="width: 60px; display: inline-block; text-align: center;">Seen</span> {{ $notification->updated_at->format('F j, Y - h:i A') }}
