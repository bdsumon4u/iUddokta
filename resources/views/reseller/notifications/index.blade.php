                    ...
                    @forelse($notifications as $notification)
                        ...
                        <div class="card shadow-sm rounded-0">
                            <div class="card-header p-2 {{ $unread ? 'border-red' : 'border--green' }}">
                                <span class="bg-secondary p-1 text-light"
                            ...
                            <div class="card-body p-2">
                                ...
                                @switch($data['notification'])
                                    @case('order-status-changed')
                                        ...
                                        Your <a target="_blank" href="{{ route('reseller.order.show', $data['order_id']) }}">Order
                                            #{{ $data['order_id'] }}</a> Status Has Changed From "{{ ucwords($data['before']) }}"
                                        To "{{ ucwords($data['after']) }}".<br>
                                        Stay With Us.<br>
                                        Thank You.
                                    @break
                                    ...
                                    @case('money-request-recieved')
                                        Dear Valuable Reseller,<br>
                                        Your <strong>Money Request #{{ $data['transaction_id'] }}</strong> For <strong>Amount
                                    ...
                                    @case('transaction-completed')
                                        ...
                                        From {{ date('d-M-Y', strtotime($timezone[0])) }} To
                                        {{ date('d-M-Y', strtotime($timezone[1])) }}<br>
                                        Stay With Us.<br>
                                        Thank You.
                                    @break
                                @endswitch
                            ...
                            @if ($unread)
                                ...
                                <div class="card-footer p-2">
                                    <div class="d-flex justify-content-between">
                                        <!-- <div class="col-md-6"> -->
                                        @if ($unread)
                                            <form action="{{ route('reseller.notifications.update', $notification->id) }}"
                                                method="post">
                                                @csrf
                                                @method('PATCH')
